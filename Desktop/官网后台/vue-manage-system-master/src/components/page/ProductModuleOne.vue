...
<script>
    import { editProductModuleOne,getProductModuleOne,addProductModuleOne } from '../../api/index';
    import UploadComponent from '../common/Upload'
    import VueEditorComponent2 from '../common/VueEditor2'
    import bus from '../common/bus';
    export default {
        name: 'ProductModuleOne',
        components: {
            UploadComponent,
            VueEditorComponent2
        },
        ...
        methods: {
            ...
            onSubmit() {
                if(this.flag_){
                    ...
                    editProductModuleOne(this.form).then(res => {
                        if(res.code == 0){
                    ...
                    this.proData = ''
                    this.editVisible = false
                    addProductModuleOne(this.form).then(res => {
                        if(res.code == 0){
                            this.$message.success(`修改成功`);
                            this.$router.go(-1)
                        }else{

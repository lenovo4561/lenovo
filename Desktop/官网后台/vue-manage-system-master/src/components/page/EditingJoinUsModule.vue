...
<script>
import { editJoinUsModuleTwo, getArticleById, getJoinUsModuleTwoById,addModuleOne,addJoinUsModuleTwo } from '../../api/index';
import UploadComponent from '../common/Upload'
import VueEditorComponent2 from '../common/VueEditor2'
import bus from '../common/bus';
export default {
    name: 'EditingJoinUsModule',
    components: {
        UploadComponent,
        VueEditorComponent2
    },
    ...
    methods: {
        ...
        getData() {
            getJoinUsModuleTwoById({id:this.form.id}).then(res => {
                if(res.code == 0){
                    this.flag_ = true
        ...
        onSubmit() {
            if(this.flag_){
                ...
                editJoinUsModuleTwo(this.form).then(res => {
                    if(res.code == 0){
                ...
                this.proData = ''
                this.editVisible = false
                addJoinUsModuleTwo(this.form).then(res => {
                    if(res.code == 0){
                        this.$message.success(`修改成功`);
                        this.$router.go(-1)
                    }else{

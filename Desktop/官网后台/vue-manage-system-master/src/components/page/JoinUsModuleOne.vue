...
<script>
    import { editJoinUsModuleOne,getJoinUsModuleOne,addJoinUsModuleOne } from '../../api/index';
    import UploadComponent from '../common/Upload'
    import VueEditorComponent2 from '../common/VueEditor2'
    import bus from '../common/bus';
    export default {
        name: 'JoinUsModuleOne',
        components: {
            UploadComponent,
            VueEditorComponent2
        },
        ...
        methods: {
            ...
            onSubmit() {
                if(this.flag_){
                    ...
                    editJoinUsModuleOne(this.form).then(res => {
                        if(res.code == 0){
                    ...
                    this.proData = ''
                    this.editVisible = false
                    addJoinUsModuleOne(this.form).then(res => {
                        if(res.code == 0){
                            this.$message.success(`修改成功`);
                            this.$router.go(-1)
                        }else{
